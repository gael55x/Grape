...
- Current-valid filtering rejects claims blocked by active claim edges: unresolved `contradicts` edges block both claims, unresolved `violates` edges block the violating source claim, and compatible `supersedes` edges block the superseded target claim until an applicable manual resolution edge says the claims may coexist or are variants. A `supersedes` edge is compatible only when the linked claims have matching claim type, resolved subject, and overlapping scope; otherwise it is warning metadata and does not suppress context.
