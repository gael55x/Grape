...
- Package-local manifests and lockfiles become dependency refs for selected package context when explicit refs identify one common package/workspace root. Task, exact-evidence, and claim sections that use that package context carry those refs so a package-local manifest or lockfile change invalidates prior sent context.
