...
- local import/export/call relationships discovered from AST traversal where supported, with bounded TypeScript compiler module resolution for relative paths, `tsconfig` path aliases, simple workspace package exports, and checker-backed direct call targets inside admitted TS/JS snapshot files
...
- JS/TS import and call resolution can miss complex package export conditions, `node_modules` dependencies, generated code, framework routing, dynamic imports, and type-checker declaration targets beyond direct calls
