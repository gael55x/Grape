...
These values are retention policy inputs. They do not run in the background. `grape compact` currently enforces context artifact retention, snapshot retention, compression input retention, FTS row retention, derived metadata retention, and invalidated-record retention. It requires `--confirm` before deleting rows or artifact files. Broader historical record limits stay as policy inputs until later compact or purge slices implement them. Invalid edited retention values fail closed instead of being silently ignored.

`grape compact` reports measured storage bytes before and after the run. The report includes `.grape/` total bytes, `grape.db`, `grape.db-wal`, `grape.db-shm`, all artifact files, artifact JSON files, artifact Markdown files, and artifact repository backing files. A preview reports the same before and after values because it deletes nothing. A confirmed run measures again after deletion. Row deletes can reduce logical storage without shrinking the SQLite database file immediately, so the byte report is measurement, not a promise that every component will go down.

Context artifact compaction deletes only eligible old `context_artifacts` rows and their cascaded artifact-owned dependency, sent, omitted, and pack rows. It also removes the matching `.json`, `.md`, and `.repository.json` files under `.grape/artifacts/` when they are regular files. It preserves the latest artifact per session, artifacts with active sent context, artifacts with restorable omitted context, artifacts with needed invalidation markers, and artifacts in locked or contended sessions.
...
Derived metadata compaction deletes only eligible `symbol_nodes` and `symbol_edges` rows by whole snapshot. It deletes symbol edges before symbol nodes. It preserves symbol metadata for the latest repo snapshot, symbol rows still referenced by surviving context artifact dependencies, and candidate snapshots that are still referenced by symbol edges from another snapshot. It does not delete source records, source files, repo snapshots, claims, proofs, source rejections, audit rows, or the SQLite database file.

Snapshot compaction deletes only eligible orphan `repo_snapshots` rows. A snapshot is eligible only when it is older than the retention window or past the row cap, is not the latest snapshot for its repo, and has no sessions, context artifacts, compression artifacts, FTS rows, symbol rows, source rows, or context dependencies. SQLite deletes the matching `worktree_states` rows through the snapshot foreign key. Compact rechecks snapshot eligibility inside the same transaction after artifact, compression, FTS, and symbol cleanup. It does not delete source records, source files, claims, proofs, project rules, source rejections, audit rows, or the SQLite database file.
