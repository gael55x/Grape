...
Monorepo support should keep package/workspace boundaries in dependency metadata before adding new schema. Current claim scope JSON may include a common-prefix `packageRoot` when an exact source ref or supported npm manifest ref is under `packages/<name>`, `apps/<name>`, `services/<name>`, or `libs/<name>`; this is scope metadata only, not a first-class package table. Current index metadata can record package roots from common manifests, and current npm manifest dependency claims can use package-root scope for current-valid filtering, but package-local manifests, lockfiles, and workspace roots still need to become complete dependency refs for selected package context before package-scoped changes can invalidate narrowly. A schema migration for first-class package/workspace tables is allowed only when tests prove metadata-only scoping is insufficient.
