...
For package-local source refs under recognized workspace prefixes (`packages/`, `apps/`, `services/`, or `libs/`), source-excerpt, parsed project-rule, symbol declaration, and package manifest dependency claim scopes record `packageRoot`. When index metadata carries manifest-backed package-root provider fields, Grape may also record nested package roots such as `components/backend` on exact source and symbol claims that use that source metadata. Supported npm manifest dependency claims use the manifest parent path as package scope because the manifest entry is the proof source. During compile, package root becomes current scope only when explicit file/task/test refs resolve to exactly one common workspace root or exactly one manifest-backed package root from index metadata. This is a package-boundary guard for current-valid claim retrieval, not proof of dependency closure, lockfile resolution, package ownership, or runtime behavior.
