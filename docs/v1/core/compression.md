...
The implemented compression slice persists deterministic `symbol_outline`, `rule_digest`, and `context_pack_summary` artifacts in SQLite through `compression_artifacts` and `compression_inputs`. Local compile builds the symbol outline from the current snapshot's lightweight symbol nodes and relationship edges, builds the rule digest from verified active rule excerpts after source-hash and excerpt-hash proof validation, and builds the context pack summary from the latest active, non-compression sent ledger rows for the current branch/head after the durable pack is written.

Only `symbol_outline` and `rule_digest` currently render in the shared `compression-orientation` section. That section is orientation only. It has no proof refs, is not pinned, is not exact-required, cannot satisfy high-risk exact context, and cannot promote durable claims. The `rule_digest` stores rule source refs, line spans, proof refs, and hashes only; exact rule text remains in the pinned active-project-rules section. The `context_pack_summary` is persisted as a cache for later use but is not rendered into artifacts yet because the current durable diff uses artifact-level manifest invalidation; rendering a ledger summary before section-level compression invalidation would make no-change turns look stale. Budget pruning now removes only optional non-safety context after required context fits, and records budget omissions explicitly. Stale compression invalidation events remain pending V1 work.
