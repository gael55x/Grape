...
Local compile also resolves current-valid durable claims and renders a `current-valid-claims` `active_claim` section when verified narrow claims are active for the current branch, commit, source hash, proof hash, and task-selected source refs. If task retrieval selects refs, unrelated active claims from the same branch/commit are omitted from the artifact even though they remain inspectable through `grape claims --active` and MCP `grape_get_claims`. The section includes claim refs, proof refs, source refs, and claim dependency rows. Current active claim types are `repository_source_excerpt_exists` and, when produced by the trusted local runner, `grape_observed_run_result`. Broader behavior, rule, decision, conflict, and correctness claims remain pending.
