...
|---|---|---|---|---|---|
| The language-indexing doc named a nonexistent provider module directory. | `docs/v1/core/language-indexing.md`; `src/core/indexing/file-index.ts`; `src/core/indexing/language-provider.ts`; `rg --files src/core/indexing`. | A beta reviewer could look for a provider tree that does not exist and overestimate how close broad language parsing is. | State that Grape currently chooses between the TS/JS parser and generic text fallback, and that provider dispatch or per-language provider modules do not exist yet. | Reassess whether the next repo-fixable blocker is provider dispatch or retrieval budget planning. | This fixes doc truth only; it does not add provider modules or language-aware graph extraction. |
