...
| Agent-facing provider capability output still did not name selected package roots. | `src/core/compiler/repository/sections/provider-capabilities.ts`; `src/core/compiler/repository/sections/builders/blind-spots.ts`; `tests/behavior/compiler/repository-context-artifact.test.mjs`; `tests/behavior/retrieval/polyglot-monorepo-fallback.test.mjs`; `docs/v1/core/language-indexing.md`. | A beta user in a monorepo could see language/provider gaps without knowing whether the selected package itself used a weak provider. | Split provider capability rendering into a section-owned helper and add selected package provider summaries from package-root metadata. | Reassess package graph limits and real MCP client trials next. | This reports selected package roots only; it does not produce complete package coverage for every package in a large workspace. |
| Agent-facing blind-spot output still hid unselected provider capability gaps. | `src/core/compiler/repository/sections/builders/blind-spots.ts`; `tests/behavior/compiler/repository-context-artifact.test.mjs`; `tests/behavior/retrieval/polyglot-monorepo-fallback.test.mjs`; `docs/v1/core/language-indexing.md`. | A beta user could ask about one Python file and miss that the repo also contains other fallback-only languages with weak graph coverage. | Add a compact indexed provider summary grouped by language and provider, with file counts, capabilities, and gaps from module-node metadata. | Reassess package graph limits and real MCP client trials next. | This is repo-level by language/provider only; it does not report complete per-package provider coverage. |
