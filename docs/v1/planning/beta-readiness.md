...
| The hosted matrix passed, but the Windows leg used a moving runner alias that emitted a redirect notice toward a newer image family. | `.github/workflows/check.yml`; GitHub Actions run `27446911045`; official `actions/runner-images` label table. | A beta package gate should not depend on a hosted runner alias that can change toolchains without repo intent. | Pin the Windows matrix leg to `windows-2025` while leaving Ubuntu and macOS on their current hosted aliases. | Watch the next hosted run and confirm the Windows redirect notice is gone. | Hosted image notices can change again; revisit the workflow if GitHub reports a new runner warning. |
...
- Hosted cross-platform CI now passes for the current alpha.3 gate, including the v6 GitHub action workflow. The Windows matrix leg now uses the pinned `windows-2025` runner label; watch the next hosted run for runner notices.
