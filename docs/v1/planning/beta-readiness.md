...
`grape-context@1.0.0-beta.9` is published on npm under the `latest` and `beta` dist-tags. `1.0.0-beta.0` remains the first beta tag and the version tied to the first committed published-package benchmark baseline. `main` may include unreleased post-beta work. Installing from npm does not include unreleased `main` behavior until the next approved publish. See `CHANGELOG.md` Unreleased and `docs/v1/planning/changelog.md` Unreleased for post-beta deltas.
...
| The MCP stdio adapter and smoke tests used `Content-Length` header framing instead of MCP newline-delimited JSON messages. | `src/mcp/protocol.ts`; `src/mcp/stdio-server.ts`; `tests/behavior/mcp/mcp-stdio.test.mjs`; packaged MCP smoke scripts. | Real MCP clients such as Claude Code and Cursor send one JSON-RPC object per line, so the server could reject the first initialize request. | Switch stdio input and output to newline-delimited JSON, reject `Content-Length` headers with a clear parse error, and update packaged smoke tests to use JSON lines. | Run the focused MCP behavior tests and the packaged beta client trial. | Real Cursor, Claude Desktop, or Claude Code UI trials still need human-owned runs before broad marketing. |
...
- `grape-context@1.0.0-beta.9` is published on npm under the `latest` and `beta` dist-tags.
...
- [x] `1.0.0-beta.9` is published on npm under the `latest` and `beta` dist-tags. `1.0.0-beta.0` remains the first beta tag and first committed published-package benchmark baseline.
...
Checked on 2026-06-25 during npm self-dogfood:
...
- npm `latest` and `beta` point at `1.0.0-beta.9`.
- npm `alpha` still points at `0.1.0-alpha.3`.
- `1.0.0-beta.9` was published at `2026-06-17T13:18:25.561Z`.
...
- `npm run benchmark:run` passed all seven gated fixtures. The stable no-change fixtures met the second-turn reduction threshold; branch-switch, dirty-worktree, stale-source, and session-reset fixtures intentionally emitted `INVALIDATE_PREVIOUS` instead of unsafe omission.
...
- **Published beta:** `grape-context@1.0.0-beta.9` is on npm under `latest` and `beta`. The first beta benchmark baseline remains tied to `1.0.0-beta.0`.
...
- Human MCP client trials in Cursor, Claude Desktop, Claude Code, or equivalent IDE UIs are not yet recorded in-repo. Automated `npm run beta:client-trial` covers packaged-install Cursor and Claude Desktop install dry-runs plus MCP stdio transport, but not a real client UI reload.
