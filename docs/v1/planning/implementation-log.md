...
- Risks or follow-ups: the compaction command still needs broader purge controls and richer export formats if future users need more than the metadata inventory.
...
- Risks or follow-ups: purge controls and richer export formats still need separate slices.
...
- Risks or follow-ups: purge controls and richer export formats still need separate slices. FTS compaction can keep rows above the configured cap when the latest repo snapshot alone exceeds the cap.
...
- Tests or checks run: focused CLI compact behavior tests and maintenance repository retention tests before broader verification.
- Risks or follow-ups: purge controls and richer export formats still need separate slices. Symbol metadata compaction can keep rows above the configured cap when the latest repo snapshot or surviving references require protection.

### 2026-06-20 - Orphan Snapshot Compaction Slice

- Author/agent: Gaille Amolong / Codex
- Principles used: evidence over assumption, small reversible increments, privacy and safety by default, and measured claims with bounded behavior.
- Summary: extended `grape compact` to enforce configured snapshot retention for orphan snapshots only. Preview reports eligible old `repo_snapshots`, and `--confirm` deletes only snapshots that are not latest for their repo and have no sessions, context artifacts, compression artifacts, FTS rows, symbol rows, source rows, or context dependencies. SQLite deletes the matching `worktree_states`. Apply rechecks snapshot eligibility inside the same transaction after artifact, compression, FTS, and symbol cleanup.
- Tests or checks run: focused CLI compact behavior tests and maintenance repository retention tests before broader verification.
- Risks or follow-ups: purge controls and richer export formats still need separate slices. Snapshot compaction can keep rows above the configured cap when snapshots still anchor source evidence or live context.

### 2026-06-20 - Invalidated Record Compaction Slice

- Author/agent: Gaille Amolong / Codex
- Principles used: evidence over assumption, small reversible increments, privacy and safety by default, and measured claims with bounded behavior.
- Summary: extended `grape compact` to enforce configured invalidated-record retention for old closed invalidation pairs. Preview reports eligible `INVALIDATE_PREVIOUS` markers and the stale sent rows they close. `--confirm` deletes a marker only when it also deletes the stale `context_sent_items` row it points at and that stale row's original pack row in the same transaction. Artifact compaction now preserves artifacts that own needed invalidation markers, so stale context cannot become active again through cleanup.
- Tests or checks run: focused CLI compact behavior tests and maintenance repository retention tests before broader verification.
- Risks or follow-ups: purge controls and richer export formats still need separate slices. Invalidated-record compaction can keep rows above the configured cap when markers are still needed to keep stale sent context inactive.
