...
- Risks or follow-ups: the compaction command still needs broader purge and export lifecycle controls.
...
- Risks or follow-ups: purge and export lifecycle controls still need separate slices.
...
- Risks or follow-ups: purge and export lifecycle controls still need separate slices. FTS compaction can keep rows above the configured cap when the latest repo snapshot alone exceeds the cap.
...
- Tests or checks run: focused CLI compact behavior tests and maintenance repository retention tests before broader verification.
- Risks or follow-ups: purge and export lifecycle controls still need separate slices. Symbol metadata compaction can keep rows above the configured cap when the latest repo snapshot or surviving references require protection.
...
- Tests or checks run: focused CLI compact behavior tests and maintenance repository retention tests before broader verification.
- Risks or follow-ups: purge and export lifecycle controls still need separate slices. Snapshot compaction can keep rows above the configured cap when snapshots still anchor source evidence or live context.
...
- Tests or checks run: focused CLI compact behavior tests and maintenance repository retention tests before broader verification.
- Risks or follow-ups: purge and export lifecycle controls still need separate slices. Invalidated-record compaction can keep rows above the configured cap when markers are still needed to keep stale sent context inactive.
