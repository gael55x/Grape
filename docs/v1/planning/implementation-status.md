...
| Local bootstrap | Tested | `grape init --connect` creates `.grape/`, writes config, applies SQLite migrations, persists a Git snapshot, and excludes `.grape/` through `.git/info/exclude`. | Add bootstrap artifact generation, candidate rules, framework/package-manager detection, and recovery tests for partial bootstrap. |
| Project scanner | In progress | Git-backed snapshot captures branch, commit, dirty paths, Git-visible files, file hashes, source-kind classification, `.gitignore`, and local privacy ignore files before reading file bytes. | Add evidence/source ingestion, size/binary handling, approval records, and explicit scan diagnostics. |
...
| Proof/evidence model | In progress | Shared trust/proof/evidence shapes exist, and durable storage has source/claim/proof tables. | Implement source ingestion, proof validation, belief gate, candidate rejection, and durable promotion rules. |
...
| Tests | Tested | `npm run check` includes docs, fixtures, memory, architecture, storage, typecheck, and 40 behavior tests including CLI bootstrap. | Add compiler/retrieval/evidence/MCP artifact-shape tests as those slices are implemented. |
| Documentation | Documented | CLI, README, roadmap/log/changelog, architecture, and this status matrix are updated for the setup slice. | Keep docs aligned as each core-pipeline slice lands. |
| Known limitations | Documented | Current docs state CLI setup exists but product context generation and MCP stdio transport remain pending. | Revisit after each slice; do not mark Done until limitations are narrowed to acceptable V1. |
