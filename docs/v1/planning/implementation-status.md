...
| Dependency tracking | Tested | Durable context build persists artifact dependency manifests and invalidates stale sent items when manifest hashes change. File relationship tracking now persists local import edges as dependency candidates. | Connect indexed relationships to repository-derived compiler selection and final artifact dependency manifests. |
...
| Tests | Tested | `npm run check` includes docs, fixtures, memory, architecture, storage, typecheck, and 49 behavior tests including CLI bootstrap, evidence source persistence, and file indexing. | Add compiler/retrieval/MCP artifact-shape tests as those slices are implemented. |
| Documentation | Documented | CLI, README, roadmap/log/changelog, architecture, storage, security, trust, and this status matrix are updated for the setup/evidence/indexing slices. | Keep docs aligned as each core-pipeline slice lands. |
| Known limitations | Documented | Current docs state CLI setup, snapshot source persistence, and lightweight file indexing exist, but product context generation, proof promotion, staged/untracked scope splitting, broad language parsing, and MCP stdio transport remain pending. | Revisit after each slice; do not mark Done until limitations are narrowed to acceptable V1. |
