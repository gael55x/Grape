...
| Task-specific context pack generation | In progress | In-memory artifact shape, durable build proof, and repository-derived artifact compiler now work from persisted snapshot/source/index inputs with deterministic hashes and risk/dirty warnings. | Add product command/tool flow, exact-span selection, final V1 JSON/Markdown artifact files, and durable session build integration from real compile requests. |
...
| Tests | Tested | `npm run check` includes docs, fixtures, memory, architecture, storage, typecheck, and 53 behavior tests including CLI bootstrap, evidence source persistence, file indexing, and repository-derived artifact compilation. | Add retrieval/MCP artifact-shape tests as those slices are implemented. |
| Documentation | Documented | CLI, README, roadmap/log/changelog, architecture, storage, security, trust, artifact contract, and this status matrix are updated for the setup/evidence/indexing/compiler-foundation slices. | Keep docs aligned as each core-pipeline slice lands. |
| Known limitations | Documented | Current docs state CLI setup, snapshot source persistence, lightweight file indexing, and repository-derived in-memory artifact compilation exist, but product context generation, proof promotion, exact-span selection, staged/untracked scope splitting, broad language parsing, and MCP stdio transport remain pending. | Revisit after each slice; do not mark Done until limitations are narrowed to acceptable V1. |
