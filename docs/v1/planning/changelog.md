...
- Added `grape purge` as a preview-first privacy cleanup command. It deletes only repo-local `.grape/` state when `--confirm` is passed, refuses symlinked state, Git-tracked `.grape/` paths, mismatched config roots, and locked or contended sessions, and leaves source files, Git history, editor config, and MCP config untouched.
- Added a dirty-worktree benchmark fixture. `grape bench --fixture dirty-worktree-typescript-app` edits a tracked source file without committing it and requires turn 2 to report dirty worktree, source-specific `INVALIDATE_PREVIOUS`, no `OMIT_UNCHANGED`, zero unsafe omissions, and zero stale sends.
- Added a no-change sync benchmark sub-gate. Token-reduction fixtures now report `noChangeSync` with benchmark id `bench_no_change_sync_time`, turn 2 to turn 1 full compile duration ratio, clean-worktree evidence, safe omission evidence, and nested failure names.
- Added a changed-file invalidation benchmark sub-gate. The stale-source fixture now reports `changedFileInvalidation` with benchmark id `bench_changed_file_invalidation_time`, turn 2 full compile duration, fixture edit-applied evidence, edited-source invalidation evidence, no `OMIT_UNCHANGED` after the edit, and nested failure names.
...
- Clarified current-vs-deferred CLI documentation: runnable SPEC/CLI examples now show implemented beta transport commands, while deferred decision and claim-linked proof commands are explicitly preserved as still-planned V1.0 surfaces.
