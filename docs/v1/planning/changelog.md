...
- Added `grape compact` for preview-first context artifact, compression cache, and FTS retention cleanup. `--confirm` is required before it deletes eligible old artifact rows, regular files under `.grape/artifacts/`, unreferenced compression cache rows, or old searchable text rows. FTS cleanup deletes whole snapshot groups and keeps the latest repo snapshot's FTS rows.
