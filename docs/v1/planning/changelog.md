...
- Added `grape compact` for preview-first context artifact, compression cache, FTS, and derived symbol metadata retention cleanup. `--confirm` is required before it deletes eligible old artifact rows, regular files under `.grape/artifacts/`, unreferenced compression cache rows, old searchable text rows, or old symbol metadata rows. FTS and symbol cleanup delete whole snapshot groups and keep the latest repo snapshot's rows.
