...
- Added `grape compact` for preview-first context artifact and compression cache retention cleanup. `--confirm` is required before it deletes eligible old artifact rows, regular files under `.grape/artifacts/`, or unreferenced compression cache rows.
