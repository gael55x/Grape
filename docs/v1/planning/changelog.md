...
- Added safe malformed-config recovery for partial bootstrap: `grape status` and `grape doctor` identify repairable local config damage, while `grape init --connect` and compile auto-bootstrap back up the invalid config before writing a fresh local config.
