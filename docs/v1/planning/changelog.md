...
- Added `grape compact` for preview-first context artifact, compression cache, FTS, derived symbol metadata, and orphan snapshot retention cleanup. `--confirm` is required before it deletes eligible old artifact rows, regular files under `.grape/artifacts/`, unreferenced compression cache rows, old searchable text rows, old symbol metadata rows, or old orphan snapshot rows. FTS and symbol cleanup delete whole snapshot groups from their own tables and keep the latest repo snapshot's rows. Snapshot cleanup deletes only snapshot rows with no live source, context, index, compression, or dependency rows.
