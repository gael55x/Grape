...
- Clarified default MCP context transport: `grape_get_context` now defaults to `outputMode: "agent_pack"` with compact pack items, `artifactRef`, compact Markdown, and an adjacency `agentGraph`; embedded full `contextArtifact` output is available only through `outputMode: "full"` or artifact inspection. MCP tool text content is a short summary rather than duplicated pretty JSON.
