...
- Clarified partial-bootstrap config recovery: malformed or project-identity-incomplete local `.grape/config.json` files are repairable by backing up the old file and writing a fresh local config, while unsupported schema versions fail closed.
