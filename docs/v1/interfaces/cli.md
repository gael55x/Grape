...
`grape compact` previews retention cleanup for eligible old context artifacts. Without `--confirm`, it deletes nothing and reports the plan. `grape compact --confirm` deletes only eligible `context_artifacts` rows plus cascaded artifact dependency, sent, omitted, and pack rows owned by those artifacts. It also removes the matching `.json`, `.md`, and `.repository.json` files under `.grape/artifacts/` when those files are regular files. It preserves the latest artifact per session, artifacts with active sent context, artifacts with restorable omitted context, and artifacts in locked or contended sessions. This first compact slice does not delete snapshots, sources, FTS rows, compression rows, claims, proofs, source rejections, audit rows, or the SQLite database file itself.
...
- `grape compact` is preview-first and requires `--confirm` before it deletes eligible local artifact rows or files.
