...
`grape compact` previews retention cleanup for eligible old context artifacts and compression cache rows. Without `--confirm`, it deletes nothing and reports the plan. `grape compact --confirm` deletes only eligible `context_artifacts` rows plus cascaded artifact dependency, sent, omitted, and pack rows owned by those artifacts. It also removes the matching `.json`, `.md`, and `.repository.json` files under `.grape/artifacts/` when those files are regular files. It preserves the latest artifact per session, artifacts with active sent context, artifacts with restorable omitted context, and artifacts in locked or contended sessions. It also deletes eligible unreferenced `compression_artifacts` rows; SQLite cascades their `compression_inputs`. It preserves compression artifacts still referenced by context artifacts that survive the same compact plan. This compact slice does not delete snapshots, sources, FTS rows, claims, proofs, source rejections, audit rows, or the SQLite database file itself.
