...
New `.grape/config.json` files carry retention defaults for artifacts, snapshots, FTS rows, compression inputs, derived metadata, and invalidated records. Older schema-1 configs without this block remain valid and read with the same defaults. `grape compact` currently enforces context artifact, compression input, FTS row, and derived metadata retention. It requires `--confirm` before deleting. Snapshot and invalidated-record limits remain policy inputs until later compact or purge slices implement them.
...
- `grape compact` is preview-first and requires `--confirm` before it deletes eligible local artifact rows, artifact files, unreferenced compression cache rows, FTS rows, or derived symbol metadata rows.
