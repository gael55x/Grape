...
New `.grape/config.json` files carry retention defaults for artifacts, snapshots, FTS rows, compression inputs, derived metadata, and invalidated records. Older schema-1 configs without this block remain valid and read with the same defaults. The CLI does not delete data from those defaults until a documented `grape compact` or `grape purge` command is implemented.
