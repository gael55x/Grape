...
- `grape compact` is preview-first and requires `--confirm` before it deletes eligible local artifact rows, artifact files, unreferenced compression cache rows, FTS rows, derived symbol metadata rows, or orphan snapshot rows.
