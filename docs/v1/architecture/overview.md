...
- Shared agent transport contracts may define the stable serialized shapes consumed by MCP and benchmark code, such as compact `agent_pack` item projection and full-output agent graph or Markdown summaries. They must not own repository IO, trust decisions, retrieval policy, or MCP adapter behavior.
...
The shared agent transport path owns only reusable serialized transport helpers: compact pack projection, the full-output MCP graph-cut shape, and full-output agent Markdown rendering for inspection. The graph cut is adjacency over already-compiled context pack items; it is not a durable graph database and must not replace proof-backed exact spans, dependency manifests, or current-valid filtering.
