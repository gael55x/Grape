...
| `INV-STATE-001` | Every new **enforced** state transition has tests. | Prevent state drift when runtime FSM is wired. | State module vocabulary; enforced paths in app/storage as implemented. | Hidden side effects bypass trust and invalidation. | `transition_matrix_covers_state_event` (required when a transition is enforced, not merely named) |
