...
| `compression_cache_used` | `context_artifact_compiled` | `compile_artifact` | artifact manifest includes compression input hashes | render artifact | context_artifacts, context_dependencies | older artifacts with changed deps | `artifact_records_compression_dependencies` |
| `compression_cache_invalidated` | `context_artifact_compiled` | `compile_without_stale_cache` | exact context available or cache regenerated | render without stale cache | context_artifacts, context_dependencies | stale cache use | `stale_cache_not_used_in_artifact` |
...
| `context_artifact_compiled` | `session_active` | `activate_session` | session identity exists and lock is acquired | load sent and omitted ledgers | context_sessions, session_events | stale session locks | `diff_requires_active_session` |
...
| `context_diff_generated` | `omitted_context_restorable` | `omit_unchanged` | item unchanged, not pinned, restore metadata exists | store omission record | omitted_context_items | none | `omitted_unchanged_has_restore` |
...
| `context_diff_generated` | `context_pack_sent` | `send_pack` | pack passes secret scan and policy validation | update sent ledger | context_sent_items, context_sessions | none | `sent_pack_persists_ledger` |
...
The current persisted proof implements the `context_artifact_compiled` -> `session_active` -> `context_diff_generated` -> `context_pack_sent` path for an already-built artifact. Local compile also implements a narrow `proof_attached` -> `proof_validated` path for repository-derived exact source/rule excerpts by validating source hash, excerpt hash, trust, privacy, redaction, and source type before inserting direct `proofs` rows. When an existing session moves branches, the durable build records `session_active` -> `session_invalidated` with `reason: "branch_changed"` and emits `previous_context_invalidated` pack items for stale sent context before sending the new pack. Durable diffing narrows dependency-manifest drift to section dependencies when stored pack-item dependency refs are available, which allows stale compression orientation to invalidate without forcing unrelated unchanged sections to resend. It intentionally does not implement full trust promotion or broad current-valid retrieval. Those states remain documented contracts until their owning goals are implemented.
