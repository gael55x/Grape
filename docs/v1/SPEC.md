...
**Runtime:** TypeScript on Node.js 22.13+ for the published package (`grape-context@beta` on npm; `alpha` remains historical)  
**Distribution:** `npm install -g grape-context@beta`  
...
| Parameter | V1 value |
|---|---|
| Runtime | Node.js 22.13+ for the published package (`grape-context@beta` on npm) |
| Language | TypeScript |
...
`grape compact` previews local retention cleanup by default. It also reports measured `.grape/`, database, WAL, SHM, artifact JSON, artifact Markdown, and artifact repository bytes before and after the run. Preview output repeats the before values because it deletes nothing. Confirmed output measures again after deletion. SQLite can retain freed pages inside `grape.db` or WAL files, so row cleanup does not guarantee smaller database file bytes immediately. `grape compact --confirm` applies the current context artifact, compression cache, FTS, derived metadata, orphan snapshot, and invalidated ledger retention slices. It deletes eligible old context artifact rows, cascaded artifact-owned rows, matching regular files under `.grape/artifacts/`, eligible unreferenced compression cache rows plus their compression input rows, eligible FTS rows by whole snapshot, and eligible `symbol_nodes` and `symbol_edges` rows by whole snapshot. It can also delete old `repo_snapshots` rows, but only when the snapshot is not the latest snapshot for its repo and has no sessions, artifacts, compression rows, FTS rows, symbol rows, source rows, or context dependencies. SQLite then deletes that snapshot's `worktree_states`. Invalidated ledger cleanup deletes old closed pairs only when the `INVALIDATE_PREVIOUS` pack row, the stale `context_sent_items` row it invalidates, and that stale row's original pack row can be removed in the same transaction. It refuses marker-only cleanup because that could make stale sent context look active again. It preserves latest-per-session artifacts, active sent context, restorable omitted context, locked or contended sessions, compression artifacts still referenced by surviving context artifacts, FTS rows for the latest repo snapshot, symbol metadata for the latest repo snapshot or surviving artifact dependencies, and artifacts that still own needed invalidation markers. FTS cleanup removes searchable text rows from `fts_entries` and `fts_entry_text`; derived metadata cleanup removes symbol index rows only. It does not delete source records, source files, claims, proofs, source rejections, audit rows, or the database file.
...
- `grape mcp --print-agents-snippet`
- `grape mcp --print-config`
- repo-local Codex plugin at `plugins/grape`
