...
`grape compact` previews local retention cleanup by default. `grape compact --confirm` applies the current context artifact, compression cache, FTS, derived metadata, and orphan snapshot retention slices. It deletes eligible old context artifact rows, cascaded artifact-owned rows, matching regular files under `.grape/artifacts/`, eligible unreferenced compression cache rows plus their compression input rows, eligible FTS rows by whole snapshot, and eligible `symbol_nodes` / `symbol_edges` rows by whole snapshot. It can also delete old `repo_snapshots` rows, but only when the snapshot is not the latest snapshot for its repo and has no sessions, artifacts, compression rows, FTS rows, symbol rows, source rows, or context dependencies. SQLite then deletes that snapshot's `worktree_states`. It preserves latest-per-session artifacts, active sent context, restorable omitted context, locked or contended sessions, compression artifacts still referenced by surviving context artifacts, FTS rows for the latest repo snapshot, and symbol metadata for the latest repo snapshot or surviving artifact dependencies. FTS cleanup removes searchable text rows from `fts_entries` and `fts_entry_text`; derived metadata cleanup removes symbol index rows only. It does not delete source records, source files, claims, proofs, source rejections, audit rows, or the database file.
