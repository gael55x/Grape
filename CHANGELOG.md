...
- Added `grape compact` for preview-first context artifact, compression cache, FTS, and derived symbol metadata retention cleanup. It requires `--confirm` before deleting eligible old artifact rows, regular artifact files, unreferenced compression cache rows, old searchable text rows, or old symbol metadata rows.
