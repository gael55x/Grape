...
- Added `grape compact` for preview-first context artifact, compression cache, and FTS retention cleanup. It requires `--confirm` before deleting eligible old artifact rows, regular artifact files, unreferenced compression cache rows, or old searchable text rows.
