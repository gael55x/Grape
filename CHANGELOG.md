...
- Added `grape mcp --install --client generic` plus `--config-path <path>` for generic JSON MCP clients and explicit config-file overrides.
...
- TypeScript/JavaScript import graph indexing now resolves relative paths, `tsconfig` path aliases, and simple workspace package `exports` from allowed snapshot files before falling back to low-confidence unresolved edges.
- TypeScript/JavaScript call graph indexing now uses a bounded TypeScript checker pass for direct call targets inside allowed snapshot files, then falls back to the older AST heuristic.
